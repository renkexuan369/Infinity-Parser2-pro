...
import requests
from utils import postprocess_doc2json_result, postprocess_doc2md_result, draw_bboxes_on_image
from prompts import *
...
def resolve_prompt(task_type, custom_prompt=None):
    """根据任务类型返回对应的 prompt"""
    if task_type == "custom":
        assert custom_prompt is not None and custom_prompt.strip(), \
...
openai_api_base = "https://jgcochhge9k5cohjme5jgqeompbh998m.openapi-hw.infly.cn"
Authorization = "PpcXBN1L0ND81LdIgOBNYGC8ILFYno+wx7BMzapDaFc="
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            with open(file_path, "rb") as f:
                ...
                async with session.post(url, data=data, headers=headers) as response:
                    # 注意：如果后续需要读取 response 内容，
                    # 建议在这里 await response.json() 或 await response.read()
...
async def doc_parser(doc_path, task_type, custom_prompt, model_id):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        all_bbox_images = [] # drawn bbox images (doc2json only), list of (PIL.Image, caption)
        ...
        for i, query in enumerate(queries):
            ...
            # 根据任务类型做后处理
            if task_type == "doc2json":
                processed = postprocess_doc2json_result(raw_page, img_paths[i], output_format='md')
